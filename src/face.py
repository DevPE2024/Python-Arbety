...
class FaceGUI:
    def __init__(self, master, strategia):
        ...
        self.losses_label = tk.Label(self.center_frame, text=f"Perdas: {self.strategia.perdas}", font=("Arial", 16), bg="#163340", fg="white")
    ...
    def update_labels(self):
        try:
            ...
            self.profit_label.config(text=f"Lucro: {self.calculate_profit()}")
            self.hits_label.config(text=f"Acertos: {self.strategia.acertos}")
            self.losses_label.config(text=f"Perdas: {self.strategia.perdas}")
        except tk.TclError:
            pass
